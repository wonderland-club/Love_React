// ------------------------------------------------------- 登入
// 登入账号 Login
export const Login_COMPONENT_NAME = "登入";
export const Login_COMPONENT_ROUTE = "/Login";
//注册账号 Register
export const Register_COMPONENT_NAME = "注册";
export const Register_COMPONENT_ROUTE = "/Register";
// ------------------------------------------------------- 小记
// 恋爱小记 LoveNotes
export const LoveNotes_COMPONENT_NAME = "恋爱小记";
export const LoveNotes_COMPONENT_ROUTE = "/";
// 添加小记 AddNote
export const AddNote_COMPONENT_NAME = "添加小记";
export const AddNote_COMPONENT_ROUTE = "/AddNote";
// ------------------------------------------------------- 历程
//恋爱历程 LoveCourse
export const LoveCourse_COMPONENT_NAME = "历程";
export const LoveCourse_COMPONENT_ROUTE = "/LoveCourse";
// 添加历程 AddJourney
export const AddJourney_COMPONENT_NAME = "添加历程";
export const AddJourney_COMPONENT_ROUTE = "/AddJourney";
// ------------------------------------------------------- 伴侣
// 查看伴侣申请 AplikasiMitra
export const AplikasiMitra_COMPONENT_NAME = "伴侣申请";
export const AplikasiMitra_COMPONENT_ROUTE = "/AplikasiMitra";
//添加伴侣 AddCompanion
export const AddCompanion_COMPONENT_NAME = "添加伴侣";
export const AddCompanion_COMPONENT_ROUTE = "/AddCompanion";
// 伴侣页面 Companion
export const Companion_COMPONENT_NAME = "伴侣";
export const Companion_COMPONENT_ROUTE = "/Companion";
// ------------------------------------------------------- 个人设置
//个人账号 Personal_Account 
export const Personal_Account_NAME = "个人账号";
export const Personal_Account_ROUTE = "/PersonalAccount";
